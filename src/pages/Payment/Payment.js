...
const Payment = () => {
  ...
  const paymentCheckout = () => {
    if (Number(userInfo.point) - totalPrice >= 0) {
      if (cartItemList) {
        ...
        //주문 완료 api 호출
        fetch(`http://${ipAddress}:3000/checkout`, {
          method: "POST",
          headers: {
        ...
        <div className="paymentInfo">
          <div className="userInfo">
            ...
            <div className="userInput">
              수령인
              {userInfo && (
                <input value={userInfo.name} disabled={true}></input>
              )}
            </div>
            <div className="userInput">
              배송지
              {userInfo && (
                <input value={userInfo.address} disabled={true}></input>
              )}
            </div>
            <div className="userInput">
              연락처
          ...
          <div className="pointInfo">
            ...
            <div className="wrapPoint">
              <div>잔여 포인트</div>
              {userInfo && (
                <span>{Number(userInfo.point).toLocaleString()} Point</span>
              )}
            </div>
            <div className="wrapPoint">
              <div>결제 포인트</div>
            ...
            <div className="wrapPoint">
              ...
              {userInfo && (
                <span>
                  {(Number(userInfo.point) - totalPrice).toLocaleString()} Point
                </span>
              )}
            </div>
          </div>
